...
| W5 | **Upstream per-tool-call timeout** (pi-subagents #150) | Part A (pin 0.48.0) landed; Part B (upstream contribution) next | OPEN — per-tool-call timeout PR merged upstream (issue filed, PR based on current main); OPEN — our `PI_SELF_HEAL=1` events observe it (verified); DONE — pinned pi-subagents re-verified to a release carrying the run-level timeout machinery (0.48.0, Part A) | Budget ≈ 1.5 pw (RICE 2.80, Blocker). Research corrected the premise: #150 is an nvm bug; the real upstream trace is our #978→#979 (run-level timeout, shipped 0.47.0+). Spec: `docs/superpowers/specs/2026-08-13-per-tool-call-timeout-upstream.md`; SCOPE.md ACTIVE. Pipeline: research ✅ → spec ✅ → scope-lock ✅ → Part A ✅ → Part B (implement) → Part C (observe/verify) → validate |
