...
| 2 | **OBS-1 — `pi-run sessions` live fleet view** — **IN IMPLEMENTATION 2026-08-16** (TDD green; PR pending) | ~10 (author-lens; band) | EPIC-2 | 0.25 pw | reader over existing `.pi/sessions/*.jsonl` v3 schema; list active (recent mtime) + recent sessions; parseable output; `--recent` default view; **CLI surface only** (no dashboard/metrics/OTel — non-goal #4); decision + debate: `.pi/debate/resilience-observability/` + `docs/knowledge-base/decision/2026-08-16-observability-first-resilience.md` |

| 3 | **OBS-2 — chat-path heal events** — **IN IMPLEMENTATION 2026-08-16** (TDD green; PR pending) | ~4.8 | EPIC-2 | 0.3–0.5 pw | aggregate repeated connection/transport failures in interactive runs → one `connection-flap` event; **hermetic-tested aggregation boundary** (≥N failures/window → exactly 1 event; <N → 0); **seam parity** (writes to existing `.pi/heal/events.jsonl` + surfaces in scorecard `selfHeal {nEvents, byKind}` — no parallel ledger); **canary acceptance** (test drives N failures → 1 `connection-flap` in `selfHeal`); CLI-only; un-gates HEAL-1/2/4 data triggers |
...
| 5 | **TAX-1 — Opt-in usage evidence via cost ledger** | ~0.7 | EPIC-6 | 0.25 pw | per-provider/per-flag counts from `.pi/cost-ledger.jsonl` (local, opt-in, no keys) → RICE Reach becomes measured |

| 6 | **EVAL-11 — Auto-open GitHub issue on N self-heals** | 0.50 | EPIC-1 | 0.5 pw | unblocked by W6; opens an issue when self-heal count crosses a threshold |

| 7 | **COST-2 — Model-catalog auto-refresh in CI** | 0.50 | EPIC-3 | 0.5 pw | hermetic drift check so routing never targets a stale/removed model |

| 8 | **EVAL-9 — Known-flaky quarantine mechanism** | 0.40 | EPIC-1 | 0.5 pw | managed flake triage with re-entry review |

| 9 | **HEAL-2 — Tune watchdog silent-window/restart-budget** | 0.35 | EPIC-2 | 0.25 pw | **data-gated**: ≥1 week non-zero `PI_SELF_HEAL` wedge coverage + W5 Part C verified (EVAL-17 is the pump) |

| 10 | **HEAL-3 — Auto-resume decision** (decision ticket) | 0.30 | EPIC-2 | 0.25 pw | record ship-or-park rationale for auto-resume after clean git-state recovery |

| 11 | **PORT-0 — Quarterly park re-confirm ticket** (replaces EPIC-4 PORT-1/PORT-2) | ~0.1 | EPIC-6 | 0.1 pw/q | re-confirm "parked, local is enough today" OR unpark on a consumer issue; all EPIC-4's DoD actually demands |
| 12 | **MSG-1 — Cross-session peer messaging for harness-launched Pi sessions** — pitch 2026-08-18; **bulletin fold-in 2026-08-21**; **PROMOTED-AS-VEHICLE 2026-08-23** (research: `docs/agent-teams-coordination-research-2026-08.md`; eval verdict: bulletin leg completed / pi-teams failed 2× startup stall → pi-bulletin is the promoted design; pi-teams removed from the project locally, remote branch pending user-confirmed deletion) | ~0.3 (author-lens ~0.8 if harness multi-session/multi-worktree runs adopt it: Reach 1 · Impact 0.5 · Conf 0.4 / 0.75 pw) | EPIC-2 | 0.75 pw | same-machine discovery (per-user on-disk registry of live harness sessions) + per-session inbox (file-based channel dirs reusing pi-subagents' `supervisor-channels` pattern; Unix-socket/CBOR via `pi-client` is the stretch alternative) + Pi-extension tools `list_agents`/`send_message` (plain text only, size cap, loop throttle, sender identity + reply address) + inbound accept/hold/refuse; **bulletin-first fold-in**: raw inboxes are the *cheap signal* layer only — add a shared digest (`bulletin_sync`) where one summarizer LLM call per round compresses concurrent findings into the shared picture, and reconcile-on-conflict (cheap symbolic diff → LLM only for unresolved claims) per the research note; **guardrails** (messages can't approve permissions, change config, or run commands; per-user socket isolation); hermetic tests (registry, delivery, refusal, throttle, digest round); **charter-conformance decision recorded 2026-08-23** (non-goal #1 "not a general-purpose agent runtime" tension — a standalone pi-bulletin OSS package is the vehicle for the coordination experiment, *not* a harness runtime feature; harness integration limited to wiring the package for harness-launched sessions, per the eval verdict in the research note) |

| 13 | **HEAL-6 — Fix `pi-run sessions --heal` filename-path mismatch — SHIPPED (28a3fa6, 2026-08-23)** — pitch 2026-08-23 (found by the pi-bulletin eval agents, verified read-only: `listSessions` discovers `<ts>_<id>.jsonl` but `scanSessionsForFlaps` reconstructs `<id>.jsonl`, so --heal silently scans nothing and reports zero flaps — `internal/cli/sessions.go:254-257`; also --recent scope ignored, duplicate flap appends, `7d` parse failure, malformed-header contract) | ~2.5 (author-lens: Reach 3 · Impact 3 · Conf 0.8 / ~0.75 pw) | EPIC-2 | 0.75 pw | fix the path reconstruction + add end-to-end tests with real timestamp-prefixed transcripts; documented in `docs/agent-teams-coordination-research-2026-08.md` eval findings |
| 14 | **SEC-1 — Provider/trust-boundary hardening — SHIPPED (d95d822, 2026-08-23)** — pitch 2026-08-23 (from the pi-bulletin HEAL-6 dogfood review): `providers.go` auto-loads project-local `providers.json` and lets a resolved API key be paired with an attacker-controlled `BaseURL` (credential exfiltration when the checkout is untrusted — devil's-advocate limited the claim, needs the trust-boundary check); `pi.go` child env inherits unrelated provider keys (least-privilege). DoD: refuse/warn on project-local provider config unless trusted; pass only the needed key to pi children; tests for both | ~2.5 (banded author-lens; raw 3·4·0.6/1 pw = 7.2 — header banding convention) | EPIC-2/3 | 1 pw | warn/refuse on untrusted project-local provider routing + child-env least-privilege; evidence in `bulletin-runs/heal6-review-3-*/digests.jsonl` |
| 15 | **BENCH-1 — Benchmark symlink traversal** — pitch 2026-08-23 (dogfood review): benchmark `copyTree` dereferences task symlinks via `os.ReadFile`, letting task content copy readable host files into the agent/Docker workspace. DoD: reject/realpath symlinked task files outside src/tests; test | ~2.0 (banded author-lens; raw 2·4·0.7/0.5 pw = 11.2 — header banding convention) | EPIC-3 | 0.5 pw | symlink-safe copy + test |
| 16 | **HEAL-7 — Subprocess timeout/kill semantics** — pitch 2026-08-23 (dogfood review): `hooks.go` timeout kills only the shell (descendants live); Windows watchdog (`watchdog_windows`) returns an error and kills nothing, then the caller waits forever; `setup.go` catalog refresh calls unbounded `execPi` despite retry language. DoD: kill the process group on timeout; fix the Windows no-op-then-wait path; bound the catalog-refresh call; tests | ~2.5 (banded author-lens; raw 3·3·0.8/1 pw = 7.2 — header banding convention) | EPIC-2 | 1 pw | group-kill + Windows fix + bounded setup call |
| 17 | **CLI-1 — Flag parsing + provider validation gaps** — pitch 2026-08-23 (dogfood review): `app.go` handwritten parsers let terminal-owned flags without values pass through as prompts/Pi args; provider JSON validation omits required-field/duplicate/unknown-field checks. DoD: owned flags reject missing values; provider config schema validation; tests | ~1.5 (banded author-lens; raw 2·3·0.7/0.75 pw = 5.6 — header banding convention) | EPIC-3 | 0.75 pw | strict owned-flag parsing + provider schema checks |
| 18 | **COST-3 — Budget-cap undercount** — pitch 2026-08-23 (dogfood review): cost ledger undercounts spend (cost.go). DoD: reconcile ledger vs session usage; add test | ~1.2 (banded author-lens; raw 2·2·0.5/0.5 pw = 4.0 — header banding convention) | EPIC-3 | 0.5 pw | ledger parity test + fix |
| *(DEFERRED — recorded decisions)* | | | | | |
| — | **MEM-1 — Memory-engine hook — CLOSED as decided (spike, 2026-08-15)** | ~1.5 | EPIC-6 | 0.5 pw (spike) | no harness memory abstraction; default = none (docs = source of truth; user choice via `.pi/settings.json`); eval runs memory-free (seam invariant); decision: `docs/knowledge-base/decision/2026-08-15-memory-engine-spike.md`; re-open on consumer signal / measured contamination / EVAL-17 data |
| 12 | **COST-1 — Static task→tier router, within-provider tier choice only — RE-OPENED 2026-08-16** (was DEFERRED 2026-08-14) | ~3.2 (author-lens, gated on COST-2; Reach ~0 externally) | EPIC-3 | 0.75 pw (corrected from 2) | **re-scoped to static `taskTiers` table + `--task` flag resolved through `resolveModelTier`, fail-closed** (NOT the cost-aware half, which stays deferred at 2 pw); re-opened via recorded owner-override (`.pi/debate/task-routing/` + `docs/knowledge-base/decision/2026-08-16-reopen-cost1-task-routing.md`); DoD: gated on **COST-2** first (stale-catalog class), `--task`/tier recorded in scorecard provenance + graded cheap-vs-balanced eval cases, fail-loud on unmapped tiers (strictly additive, no-`--task` = today's behavior), adoption data gate; cost-aware half + cross-provider still out of scope (charter NOT-6, 2026-08-12 cost-aware-routing design as basis); **`--task` never applies to `eval`/`ci-benchmark` runs** (benchmark keeps `--provider`/`--model` only — scorecard comparability preserved); promotion to active ROADMAP is a user decision


| *(SHIPPED — see CHANGELOG)* | | | | | |
| 5 | **EVAL-12 — Live re-baseline after W10 — SHIPPED (#140, 2026-08-15)** | 1.10 | EPIC-1 | 0.25 pw | re-baseline 17 → 55 cases from the first complete 55-case nightly; **DoD: 0 unbaselined live cases**, 0 incomplete omitted; provenance recorded (agentModel cheap / judgeModel gpt-4.1-mini); 14 sub-1.0 rates as honest bounds; standing re-baseline cadence established |
| 1 | **EVAL-16 — Harness-change eval gate — pilot SHIPPED (#129)** | ~1.6 | EPIC-1 | 0.3–0.5 pw | score_delta.py (classifier + delta renderer) + 15 hermetic tests + report-only eval-delta CI job + nightly delta artifact; **enforcement pending** (evidence-gated: first caught regression or validated delta-vs-noise; v1.0.0 gate "EVAL-16 enforced" still open) |
