...
| W6 | **Scorecard self-heal observability** (surface `PI_SELF_HEAL` events + enable in CI) | In progress — implementation done, landing via PR | DONE — `nightly-live-eval.yml` live job sets `PI_SELF_HEAL=1`; DONE — `score_run.py` summary/compact/step-summary surface `selfHeal {nEvents, byKind}` (hermetic tests in deterministic CI); OPEN — merged + first nightly records events + DoD closed in CHANGELOG | Budget ≈ 0.5 pw (RICE 1.40, Enabler). Unblocks BACKLOG #4/#5. Spec: `docs/superpowers/specs/2026-08-13-surface-self-heal-events-scorecard.md`; SCOPE.md ACTIVE. Pipeline: promote ✅ → spec ✅ → scope-lock ✅ → TDD ✅ → PR → verify |
