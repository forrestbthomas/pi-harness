...
| W5 | **Upstream per-tool-call timeout** (upstream #1076/#1077; run-level trace #978/#979) | Part A (pin 0.48.0) landed; Part B submitted upstream — issue #1076, PR #1077 open (2026-08-13); Part C next | OPEN — per-tool-call timeout PR merged upstream (#1077 submitted, awaiting maintainer review); OPEN — our `PI_SELF_HEAL=1` events observe it (Part C, after a release carries `toolTimeoutMs`); DONE — pinned pi-subagents re-verified to a release carrying the run-level timeout machinery (0.48.0, Part A) | Budget ≈ 1.5 pw (RICE 2.80, Blocker). #150 was a false lead (nvm bug); the real trace is our #978→#979 (run-level timeout, shipped 0.47.0+) and now #1076/#1077 (per-tool timeout). Spec: `docs/superpowers/specs/2026-08-13-per-tool-call-timeout-upstream.md`; SCOPE.md ACTIVE (file-surface expansion approved 2026-08-13: chain-execution.ts + parallel-utils.ts). Pipeline: research ✅ → spec ✅ → scope-lock ✅ → Part A ✅ → Part B (implement + PR) ✅ submitted → Part C (observe/verify) → validate |
