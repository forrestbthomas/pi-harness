...
| W7 | **Flake-aware gate + evidence artifacts** (EVAL-1/EVAL-2, EPIC-1) | In progress — implementation done, landing via PR | DONE — upload steps `if: always()` + `.pi/heal/events.jsonl` in nightly artifact (EVAL-1); DONE — flake-aware gate: 1-of-N flake warn-only, ≥2 fails = regression, `flakes` in summary/compact/markdown, n=5 (EVAL-2); OPEN — merged + next live nightly verifies end-to-end + DoD closed | Budget ≈ 0.75 pw (EVAL-1 0.25 + EVAL-2 0.5; EPIC-1). Spec: `docs/superpowers/specs/2026-08-14-flake-aware-gate-and-evidence-artifacts.md`; SCOPE.md ACTIVE. Pipeline: promote ✅ → spec ✅ → scope-lock ✅ → TDD ✅ → PR → verify |
