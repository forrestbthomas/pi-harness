...
| W5 | **Upstream per-tool-call timeout** (pi-subagents #150) | In research (shaping) — Phase 1: librarian research on the upstream tool-call loop | OPEN — per-tool-call timeout PR merged upstream (pi-subagents #150); OPEN — our `PI_SELF_HEAL=1` events observe it (verified); OPEN — pinned pi-subagents re-verified to a release carrying the timeout | Budget ≈ 1.5 pw (RICE 2.80, Blocker). The one self-healing piece that belongs upstream: bounds a single subagent tool call so mid-tool wedges can't hang with output flowing (the gap our run-level watchdog can't see). Our prior #978/#979 (async-timeout) merged upstream; new work composes with them. Follow pipeline: research → spec (`spec-plan`) → scope-lock → implement (TDD) → validate |
