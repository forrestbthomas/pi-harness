...
|---|---|---|---|---|---|
| 1 | **GOV-2 — Governance relocation + spec archive + BACKLOG table repair** | ~2.5 | EPIC-6 | 0.25–0.5 pw | move root PM docs → `docs/governance/`; archive 19 dated specs → decisions file; ranks 8–23 repaired (2026-08-14). **GOV-1 shipped first (#112) per SCOPE.md scope change #1** — sequencing text updated 2026-08-14 |

| 2 | **OSS-2 — Contributor on-ramp v2** | ~2.0 | EPIC-6 | 0.5 pw | first-issue path, PR-template bugfix carve-out (no ROADMAP citation for in-scope fixes), review SLA mirroring SECURITY 7-day, MIT-in/MIT-out line |

| 3 | **OWN-1 — CODEOWNERS ownership matrix + fallback-to-lead** | ~1.5 | EPIC-6 | 0.25 pw | real matrix (Go core, /eval, .github, scripts/) + explicit fallback; seeds the charter's second-owner split trigger |

| 4 | **EVAL-16 — Harness-change eval gate (pilot-first)** | ~1.2 | EPIC-1 | 0.5 pw | scorecard-delta on eval-touching PRs; pilot after W5 Part C; promote to enforced gate on first caught regression (zero-token silent-success class) |
...
| 6 | **EVAL-12 — Live re-baseline after W10** (scheduled: tonight after the 03:00 UTC nightly) | 1.10 | EPIC-1 | 0.25 pw | pull live-results artifact, review the 54-case report, commit `--update-baseline`; **DoD: 0 unbaselined live cases** + standing re-baseline cadence (after every dataset/harness change; history in `eval/baselines/`) |

| 7 | **SECURITY — supported-versions bump in release ritual** | ~1.0 | EPIC-6 | 0.25 pw | same PR as CHANGELOG entry bumps the supported-versions table (SECURITY.md) |

| 8 | **COST-1 — Cost-aware router, within-provider tier choice only** | 0.80 | EPIC-3 | 2 pw | per-task-tier model choice **within the explicit provider**; cross-provider routing explicitly out of scope (charter NOT-6) + decision ticket |

| 9 | **TAX-1 — Opt-in usage evidence via cost ledger** | ~0.7 | EPIC-6 | 0.25 pw | per-provider/per-flag counts from `.pi/cost-ledger.jsonl` (local, opt-in, no keys) → RICE Reach becomes measured |

| 10 | **EVAL-11 — Auto-open GitHub issue on N self-heals** | 0.50 | EPIC-1 | 0.5 pw | unblocked by W6; opens an issue when self-heal count crosses a threshold |

| 11 | **COST-2 — Model-catalog auto-refresh in CI** | 0.50 | EPIC-3 | 0.5 pw | hermetic drift check so routing never targets a stale/removed model |

| 12 | **EVAL-9 — Known-flaky quarantine mechanism** | 0.40 | EPIC-1 | 0.5 pw | managed flake triage with re-entry review |

| 13 | **HEAL-2 — Tune watchdog silent-window/restart-budget** | 0.35 | EPIC-2 | 0.25 pw | **data-gated**: ≥1 week non-zero `PI_SELF_HEAL` wedge coverage + W5 Part C verified (EVAL-6 is the pump) |

| 14 | **HEAL-3 — Auto-resume decision** (decision ticket) | 0.30 | EPIC-2 | 0.25 pw | record ship-or-park rationale for auto-resume after clean git-state recovery |

| 15 | **PORT-0 — Quarterly park re-confirm ticket** (replaces EPIC-4 PORT-1/PORT-2) | ~0.1 | EPIC-6 | 0.1 pw/q | re-confirm "parked, local is enough today" OR unpark on a consumer issue; all EPIC-4's DoD actually demands |
