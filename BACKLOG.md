...
**Owner:** forrestthomas · **Last updated:** 2026-08-18
...
| 14 | **SEC-1 — Provider/trust-boundary hardening — SHIPPED (d95d822, 2026-08-23)** — pitch 2026-08-23 (from the pi-bulletin HEAL-6 dogfood review): `providers.go` auto-loads project-local `providers.json` and lets a resolved API key be paired with an attacker-controlled `BaseURL` (credential exfiltration when the checkout is untrusted — devil's-advocate limited the claim, needs the trust-boundary check); `pi.go` child env inherits unrelated provider keys (least-privilege). DoD: refuse/warn on project-local provider config unless trusted; pass only the needed key to pi children; tests for both | ~2.5 (author-lens: Reach 3 · Impact 4 · Conf 0.6 / ~1 pw) | EPIC-2/3 | 1 pw | warn/refuse on untrusted project-local provider routing + child-env least-privilege; evidence in `bulletin-runs/heal6-review-3-*/digests.jsonl` |
| 15 | **BENCH-1 — Benchmark symlink traversal** — pitch 2026-08-23 (dogfood review): benchmark `copyTree` dereferences task symlinks via `os.ReadFile`, letting task content copy readable host files into the agent/Docker workspace. DoD: reject/realpath symlinked task files outside src/tests; test | ~2.0 (Reach 2 · Impact 4 · Conf 0.7 / ~0.5 pw) | EPIC-3 | 0.5 pw | symlink-safe copy + test |
| 16 | **HEAL-7 — Subprocess timeout/kill semantics** — pitch 2026-08-23 (dogfood review): `hooks.go` timeout kills only the shell (descendants live); Windows watchdog (`watchdog_windows`) returns an error and kills nothing, then the caller waits forever; `setup.go` catalog refresh calls unbounded `execPi` despite retry language. DoD: kill the process group on timeout; fix the Windows no-op-then-wait path; bound the catalog-refresh call; tests | ~2.5 (Reach 3 · Impact 3 · Conf 0.8 / ~1 pw) | EPIC-2 | 1 pw | group-kill + Windows fix + bounded setup call |
| 17 | **CLI-1 — Flag parsing + provider validation gaps** — pitch 2026-08-23 (dogfood review): `app.go` handwritten parsers let terminal-owned flags without values pass through as prompts/Pi args; provider JSON validation omits required-field/duplicate/unknown-field checks. DoD: owned flags reject missing values; provider config schema validation; tests | ~1.5 (Reach 2 · Impact 3 · Conf 0.7 / ~0.75 pw) | EPIC-3 | 0.75 pw | strict owned-flag parsing + provider schema checks |
| 18 | **COST-3 — Budget-cap undercount** — pitch 2026-08-23 (dogfood review): cost ledger undercounts spend (cost.go). DoD: reconcile ledger vs session usage; add test | ~1.2 (Reach 2 · Impact 2 · Conf 0.5 / ~0.5 pw) | EPIC-3 | 0.5 pw | ledger parity test + fix |
